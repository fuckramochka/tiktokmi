...
public final class Cats {
    ...
    private static void meow(final Context context) {
        try {
            ...
            final File sound = new File(context.getFilesDir(), "tiktokmi/meow.ogg");
            if (!sound.isFile()) {
                java.io.InputStream in = context.getAssets().open("tiktokmi/meow.ogg");
    ...
    private static void fetch(final Context context) {
        ...
        final File cache = new File(context.getFilesDir(), "tiktokmi/cats.json");
    ...
    private static byte[] picture(Context context, String path) {
        File cache = new File(context.getFilesDir(), "tiktokmi/cats/" + name(path));
        byte[] have = Net.read(cache);
