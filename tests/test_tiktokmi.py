...
from tiktokmi import (accent as accent_module, artwork, dexpatch, icon as icon_module,
                    manifest as manifest_module, palette, png, vector)
from tiktokmi.apkzip import Apk, STORED
from tiktokmi.arsc import Arsc, ArscError
from tiktokmi.axml import Axml, TYPE_REFERENCE, TYPE_STRING
...
class AxmlTest(unittest.TestCase):
    ...
    def test_label_becomes_an_inline_string(self):
        axml = Axml.parse(self.raw)
        manifest_module.set_label(axml, "TikTok MI")
        ...
        self.assertEqual(again.pool.get(attr.data), "TikTok MI")
    ...
    def test_added_activity_survives_a_rebuild(self):
        ...
        axml = Axml.parse(self.raw)
        manifest_module.set_label(axml, "TikTok MI")
        manifest_module.add_activity(axml, "mi.tiktokmi.SettingsActivity",
                                     "TikTok MI settings", 0x0103012C)
        again = Axml.parse(axml.build())

        names = [again.attr_string(node, "name") for node in again.elements("activity")]
        self.assertIn("mi.tiktokmi.SettingsActivity", names)
        self.assertIn("cat.narezany.fixture.MainActivity", names)

        added = [n for n in again.elements("activity")
                 if again.attr_string(n, "name") == "mi.tiktokmi.SettingsActivity"][0]
        self.assertEqual(again.attr_string(added, "label"), "TikTok MI settings")
        ...
        self.assertNotIn("mi.tiktokmi.SettingsActivity", launchers)
    ...
    def test_a_launcher_activity_gets_a_task_of_its_own(self):
        ...
        axml = Axml.parse(self.raw)
        manifest_module.add_activity(axml, "mi.tiktokmi.SettingsActivity",
                                     "TikTok MI settings", 0x0103012C, "mi.tiktokmi",
                                     launcher=True)
        again = Axml.parse(axml.build())

        added = [n for n in again.elements("activity")
                 if again.attr_string(n, "name") == "mi.tiktokmi.SettingsActivity"][0]
        self.assertEqual(again.attr_string(added, "taskAffinity"), "mi.tiktokmi")
        self.assertEqual(again.attr(added, "launchMode").data,
                         manifest_module.LAUNCH_SINGLE_TASK)
        launchers = [again.attr_string(n, "name") for n in manifest_module.launcher_elements(again)]
        self.assertIn("mi.tiktokmi.SettingsActivity", launchers)
        self.assertIn("cat.narezany.fixture.Splash", launchers)

    def test_a_provider_can_be_declared(self):
        axml = Axml.parse(self.raw)
        manifest_module.add_provider(axml, "mi.tiktokmi.MargyProvider",
                                     "cat.narezany.fixture.tiktokmi")
        again = Axml.parse(axml.build())
        providers = {again.attr_string(n, "name"): n for n in again.elements("provider")}
        self.assertIn("mi.tiktokmi.MargyProvider", providers)
        added = providers["mi.tiktokmi.MargyProvider"]
        self.assertEqual(again.attr_string(added, "authorities"), "cat.narezany.fixture.tiktokmi")
    ...
    def test_the_screen_the_row_goes_on_is_the_one_the_mod_looks_for(self):
        ...
        from tiktokmi.build import TIKTOK_SETTINGS
        source = os.path.join(ROOT, "inject", "java", "mi", "tiktokmi",
                              "SettingsRow.java")
    ...
    def test_only_the_authorities_the_package_does_not_cover_move(self):
        ...
        renames = {old: old + ".tiktokmi" for old in shared}
        ...
        self.assertIn("com.example.shared.provider1233.tiktokmi", authorities)
        ...
        self.assertEqual(manifest_module.shared_authorities(again, package),
                         ["com.example.shared.provider1233.tiktokmi"])
...
"""
    ...
    def test_every_target_has_a_method_to_land_in(self):
        source = os.path.join(ROOT, "inject", "java", "mi", "tiktokmi", "Region.java")
        with open(source, encoding="utf-8") as handle:
            java = handle.read()
    ...
    def test_a_renamed_authority_moves_in_the_bytecode_too(self):
        ...
        try:
            ...
            counts = dexpatch.rewrite_literals(
                room, {"com.example.shared.provider1233": "com.example.shared.provider1233.tiktokmi"})
            self.assertEqual(counts, {"com.example.shared.provider1233": 1})
            with open(os.path.join(room, "a.smali"), encoding="utf-8") as handle:
                out = handle.read()
            self.assertIn('"com.example.shared.provider1233.tiktokmi"', out)
    ...
    def test_the_accent_constant_becomes_a_call(self):
        ...
        self.assertIn("invoke-static {}, Lmi/tiktokmi/Accent;->accent()I", text)
    ...
    def test_the_save_button_is_pointed_at_the_clean_address(self):
        ...
        for _label, pattern, target in dexpatch.model_rules():
            text = pattern.sub(target, text)
        self.assertIn("invoke-static {v3}, Lmi/tiktokmi/Download;->getDownloadAddr("
    ...
    def test_the_feed_page_comes_through_the_mod(self):
        ...
        for _label, pattern, target in dexpatch.model_rules():
            text = pattern.sub(target, text)
        self.assertIn("Lmi/tiktokmi/Feed;->getItems("
    ...
    def test_a_field_read_becomes_a_call_and_a_move(self):
        ...
        for _label, pattern, target in dexpatch.model_rules():
            text = pattern.sub(target, text)
        self.assertIn("invoke-static {v5}, Lmi/tiktokmi/Download;->allowDownload("
    ...
    def test_a_colour_being_applied_is_redirected_too(self):
        ...
        for _label, pattern, target in dexpatch.accent_rules():
            text = pattern.sub(target, text)
        self.assertIn("Lmi/tiktokmi/Accent;->setColor(Landroid/graphics/Paint;I)V", text)
        self.assertIn("Lmi/tiktokmi/Accent;->setTextColor("
                      "Landroid/widget/TextView;I)V", text)
        # a static keeps its shape exactly: no receiver to move
        self.assertIn("invoke-static {v1}, Lmi/tiktokmi/Accent;->valueOf(I)"
    ...
    def test_the_stamp_is_only_dropped_in_the_class_that_draws_it(self):
        ...
        self.assertIn("Lmi/tiktokmi/Watermark;->drawBitmap("
        ...
        self.assertNotIn("tiktokmi", other)  # a class without the marker is left alone
    ...
    def test_a_colour_asked_of_the_framework_is_redirected(self):
        ...
        for _label, pattern, target in dexpatch.accent_rules():
            text = pattern.sub(target, text)
        self.assertIn("invoke-static {v0, v1}, Lmi/tiktokmi/Accent;->"
