...
# The screen the TikTok MI row is put at the top of. The row itself is added to
...
SETTINGS_LABEL = "TikTok MI settings"
...
class Build:
    ...
    def run(self) -> str:
        ...
        if not manifest_module.has_activity(manifest, TIKTOK_SETTINGS):
            raise RuntimeError(
                ...
                "renamed, and the TikTok MI row would never appear in it"
                % TIKTOK_SETTINGS
            )
        ...
        manifest_module.add_provider(
            manifest, STARTUP_PROVIDER, "%s.tiktokmi" % package)
        self.detail("%s declared: the mod starts with the app" % STARTUP_PROVIDER)
        self.detail("the TikTok MI row goes on top of %s" % TIKTOK_SETTINGS.rsplit(".", 1)[-1])
    ...
    def write_baked_colour(self, moved: Dict[int, int]) -> None:
        ...
        path = os.path.join(self.root, "inject", "java", "mi", "tiktokmi",
                            "Baked.java")
    ...
    def write_version(self, tiktok: str) -> None:
        ...
        out = os.path.join(self.root, "inject", "java", "mi", "tiktokmi",
                           "Version.java")
        ...
        self.detail("TikTok MI %s on TikTok %s" % (mod, tiktok))
    ...
    def add_assets(self, apk: Apk) -> None:
        ...
        for name in ("twemoji.ttf", "noto.ttf", "blobmoji.ttf", "meow.ogg"):
            ...
            apk.add("assets/tiktokmi/" + name, raw, STORED)
            self.detail("assets/tiktokmi/%s: %d kB" % (name, len(raw) // 1024))
